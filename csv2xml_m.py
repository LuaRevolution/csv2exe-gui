...
errors = ""
class argumentList:
    def __init__(self,source_model_name=None,csv_file_name=None,output_file_path=None):
        self.source_model_name = source_model_name
        self.csv_file_name = csv_file_name
        self.output_file_path = output_file_path
def generate(csv_file_name,output_file_path,source_model_name):
    global errors
    errors = ""
    simple_fault_rows_order = [[False, "source id", "source name", "tectonicRegion"],
    						  [True, "faultGeometry latitude", "faultGeometry longitude"],
    ...
    # 							"hypo alongStrike", "hypo downDip", "hypo weight", "slip value", "slip weight","YoungsCoppersmithMFD minmag"]


    simplefault_common_rows = ["source type", "source id", "source name", "faultGeometry latitude",
    							"faultGeometry longitude", "dip", "upperSeismoDepth", "lowerSeismoDepth",
    							"magScaleRel", "ruptAspectRatio", "rake",
    ...
    args = argumentList(csv_file_name=csv_file_name,output_file_path=output_file_path,source_model_name=source_model_name)
    ...
    def area_source_generator(i, source_element):
    	...
    	if rows[i][0] == 'areageometry latitude':
    		...
    		Poligon = etree.SubElement(areaGeometry, 'gmlPolygon')
    	...
    	if rows[i][0] == 'truncgutenbergrichtermfd avalue':
    		...
    		for id in numpy.arange(2,len(rows[i]),1):
    			if rows[i][id] != '' :
    				...
    				truncGutenbergRichterMFD.attrib['bValue'] = rows[i+1][id]
    ...
    #	if rows[i][0] == 'youngscoppersmithmfd minmag':
    #		YoungsCoppersmithMFD = etree.SubElement(source_element, 'YoungsCoppersmithMFD')
    #		for id in numpy.arange(2,len(rows[i]),1):
    #			if rows[i][id] != '' :
    #				# აქ ხდება hypo ტეგის დამატება hypolist -ის ქვეელემენტად და ივსება მისი ატრიბუტები
    #				YoungsCoppersmithMFD.attrib['minmag'] = rows[i][id]
    #				YoungsCoppersmithMFD.attrib['bValue'] = rows[i+1][id]
    #				YoungsCoppersmithMFD.attrib['binWidth'] = rows[i+2][id]
    #				YoungsCoppersmithMFD.attrib['characteristicMag'] = rows[i+3][id]
    #				YoungsCoppersmithMFD.attrib['characteristicRate'] = rows[i+4][id]
    	# END EDIT ---------------------------------------------------------
    	...
    	if rows[i][0] == 'slip value':
    		...
    		for id in numpy.arange(2,len(rows[i]),1):
    			if rows[i][id] != '' :
    				...
    				slip.text = rows[i][id]
    	#tu shegxvda YoungsCoppersmithMFD   es nawili dasasrulebelia da dasamatebelia chekebshi
    	if rows[i][0] == 'youngscoppersmithmfd minmag':
    		YoungsCoppersmithMFD = etree.SubElement(source_element, 'YoungsCoppersmithMFD')
    		YoungsCoppersmithMFD.attrib['minMag'] = rows[i][2]

    	if rows[i][0] == 'youngscoppersmithmfd bvalue':
    		YoungsCoppersmithMFD.attrib['bValue'] = rows[i][2]

    	if rows[i][0] == 'youngscoppersmithmfd binwidth':
    		YoungsCoppersmithMFD.attrib['binWidth'] = rows[i][2]

    	if rows[i][0] == 'youngscoppersmithmfd characteristicmag':
    		YoungsCoppersmithMFD.attrib['characteristicMag'] = rows[i][2]

    	if rows[i][0] == 'youngscoppersmithmfd characteristicrate':
    		YoungsCoppersmithMFD.attrib['characteristicRate'] = rows[i][2]

    	if rows[i][0] == 'youngscoppersmithmfd totalmomentrate':
    		YoungsCoppersmithMFD.attrib['totalMomentRate'] = rows[i][2]
    ...
    def complex_fault_generator(i, source_element):
    	...
    	if rows[i][0] == 'faulttopedge latitude':
    		...
    		for id in numpy.arange(2, len(rows[i]), 1):
    			if rows[i][id] !=  '':
    				lat_long_elev_pairs += '\n            ' + rows[i+1][id] + ' ' + rows[i][id] + ' ' + rows[i+2][id]
...
if __name__ == "__main__":
    generate("D:\Work Stuff\OpenQuake UI\other project\example\\test_file_simple_faults.csv","/example/","simpleFaultSource")
